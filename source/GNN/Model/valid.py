...
from utils.pytorchtools_seq2seq import EarlyStopping

from utils.data.dataset import SantanderDataset
from utils.data.dataloader import SantanderDataloader
...
from setting_param import Model_InputDir, Model_main_worker, Model_main_batchSize, Model_main_state_dim, Model_main_output_dim, Model_main_n_steps, Model_main_init_L, Model_main_niter, Model_main_lr
...
def main(opt):
    test_dataset = SantanderDataset(opt.dataroot, opt.L, False, False)
    ...
    # annotationをPM2.5だけにしておくと精度が上がった（その他の特徴量は逆にいらない？）
    ...
    return test(test_dataloader, encoder, decoder, criterion, opt)

test_loss, abs_target, abs_output = main(opt)
