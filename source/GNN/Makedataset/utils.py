...
def data_iterator(data,timestamps,window_size = 48,slide =8,out_size=8):
    ...
    return (
        [
            np.array([data[j:j + window_size] for j in range(8500, len(data)-12000, slide)]),
            np.array([standard_time[j + window_size + 1:j + window_size + 1 + out_size] for j in range(8500, len(data)-12000, slide)])

            #np.array([data[j:j+window_size] for j in range(0,len(data) - window_size - out_size,slide)]),
            #np.array([standard_time[j + window_size + 1:j + window_size + 1 + out_size] for j in range(0,len(data) - window_size - out_size,slide)])

        ],
        np.array([data[j + window_size + 1:j + window_size + 1 + out_size] for j in range(8500, len(data)-12000, slide)])
